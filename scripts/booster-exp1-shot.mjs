// Headed shot of booster-exp1 snapshot. Titan on the booster is expected.
...
const SNAP = join(ROOT, 'logs/snapshots/booster-exp1.json');
const RESULT = join(ROOT, 'logs/booster-exp1.json');
...
try {
  ...
  const out = join(SHOT_DIR, 'booster-exp1.png');
  ...
  console.log(last.real3d ? '3D' : 'WEAK', 'booster-exp1.png', last.bytes, 'scene', last.sceneBytes, 'comp', last.composited);
} finally {
  await Promise.race([
