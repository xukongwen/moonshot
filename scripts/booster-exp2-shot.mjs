// Headed shot of booster-exp2 snapshot. Stock Mun Express Titan booster.
...
const SNAP = join(ROOT, 'logs/snapshots/booster-exp2.json');
const RESULT = join(ROOT, 'logs/booster-exp2.json');
...
try {
  ...
  const out = join(SHOT_DIR, 'booster-exp2.png');
  ...
  console.log(last.real3d ? '3D' : 'WEAK', 'booster-exp2.png', last.bytes, 'scene', last.sceneBytes, 'comp', last.composited);
} finally {
  await Promise.race([
