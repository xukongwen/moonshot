// Headed shot of booster-exp3 snapshot. Stock Mun Express Titan booster.
...
const SNAP = join(ROOT, 'logs/snapshots/booster-exp3.json');
const RESULT = join(ROOT, 'logs/booster-exp3.json');
...
try {
  ...
  const out = join(SHOT_DIR, 'booster-exp3.png');
  ...
  console.log(last.real3d ? '3D' : 'WEAK', 'booster-exp3.png', last.bytes, 'scene', last.sceneBytes, 'comp', last.composited);
} finally {
  await Promise.race([
