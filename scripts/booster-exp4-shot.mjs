// Headed shot of booster-exp4 snapshot. Stock Mun Express Titan booster. Stock Mun Express Titan booster.
...
const SNAP = join(ROOT, 'logs/snapshots/booster-exp4.json');
const RESULT = join(ROOT, 'logs/booster-exp4.json');
...
const server = await createServer({
  root: ROOT,
  server: { port: 5222, strictPort: false, host: '127.0.0.1' },
});
await server.listen();
...
try {
  ...
  const out = join(SHOT_DIR, 'booster-exp4.png');
  ...
  console.log(last.real3d ? '3D' : 'WEAK', 'booster-exp4.png', last.bytes, 'scene', last.sceneBytes, 'comp', last.composited);
} finally {
  await Promise.race([
