// Headed proof: Mun Express OX-STAT coplanar wings (same face, both grids).
...
import { fillEC, sunVectorInertial, panelNormal, ecTelemetry, wheelsLive } from '../src/power.js';

const ROOT = dirname(fileURLToPath(new URL('../package.json', import.meta.url)));
const SHOT_DIR = join(ROOT, 'logs/shots');
const OUT_PNG = join(SHOT_DIR, 'panel-coplanar.png');
const OUT_JSON = join(ROOT, 'logs/panel-coplanar.json');
...
/** Day-side 80 km. Shared face pointed at Kerbol; wings stay a T in vessel frame. */
function placeDay({ t = 0, alt = 80_000 } = {}) {
  ...
  fillEC(st);
  // Shared face at Kerbol so both wings generate (~1.6) and both grids face the camera.
  const sun = sunVectorInertial(st, t);
  const panel = st.parts.find((p) => p.def.panel);
  const bodyN = panelNormal({ quat: new Quaternion() }, panel, 0);
...
// Slightly above the shared face: along the painted normal, up the stack,
// tiny side so both ±span wings read as a T (two blue grids, not one back).
const CAMS = [
  { out: 5.2, up: 3.4, side: 0.35 },
  { out: 4.6, up: 4.2, side: 0.25 },
  { out: 6.0, up: 2.8, side: 0.45 },
  { out: 5.0, up: 5.0, side: 0.20 },
];
...
try {
  ...
  const placed = await page.evaluate((p) => {
    ...
    if (!f._pairFrozen) {
      ...
      f.updateScene = (dt) => {
        origUpdate(dt);
        const cam = f._pairCam || { out: 5.2, up: 3.4, side: 0.35 };
        ...
        const a0 = Number.isFinite(panel.attachAngle) ? panel.attachAngle : 0;
        const faceWorld = new V3(-Math.sin(a0), 0, Math.cos(a0)).applyQuaternion(st.quat).normalize();
        const yWorld = new V3(0, 1, 0).applyQuaternion(st.quat).normalize();
        const sideWorld = new V3().crossVectors(faceWorld, yWorld).normalize();
        const out = Number.isFinite(cam.out) ? cam.out : 5.2;
        const up = Number.isFinite(cam.up) ? cam.up : 3.4;
        const side = Number.isFinite(cam.side) ? cam.side : 0.35;
        const camPos = world.clone()
          .addScaledVector(faceWorld, out)
          .addScaledVector(yWorld, up)
          .addScaledVector(sideWorld, side);
  ...
  console.log(last.real3d ? '3D' : 'WEAK', 'panel-coplanar.png', last.bytes, 'scene', last.sceneBytes, 'comp', last.composited);
...
const result = {
  ...
  setup: 'stock hang sym 2 at 90°/270°, shared face at Kerbol, SAS off, legs stowed, coplanar wings',
  method: 'headed Chrome channel=chrome canvas.toDataURL + HUD colorkey overlay',
  cam: usedCam,
  path: 'logs/shots/panel-coplanar.png',
  bytes: last?.bytes ?? 0,
...
try { copyFileSync(OUT_PNG, '/workspace/panel-coplanar.png'); } catch { /* optional */ }
