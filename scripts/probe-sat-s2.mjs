#!/usr/bin/env node
// Headed proof: Kerbin Eye day-side 80 km, SAS hold (S2).
...
const OUT_PNG = join(SHOT_DIR, 'sat-s2-hold.png');
const OUT_JSON = join(ROOT, 'logs/sat-s2.json');
...
/** Day-side 80 km. Identity quat so ±Z wings sit left-right. SAS hold. */
...
async function runOnce() {
  const server = await createServer({
    root: ROOT,
    server: { port: 5234, strictPort: false, host: '127.0.0.1' },
  });
  await server.listen();
  ...
  try {
    ...
    const placed = await page.evaluate((p) => {
      ...
      // SAS hold a couple of seconds before the beauty freeze.
      if (typeof f.physStep === 'function') {
        for (let i = 0; i < 150; i++) f.physStep(0.02);
      }
      st.sas = true;
      st.sasMode = 'hold';
      st.ec = p.ec;
    ...
    console.log(last.real3d ? '3D' : 'WEAK', 'sat-s2-hold.png', last.bytes, 'scene', last.sceneBytes, 'comp', last.composited);
    ...
    const live = await page.evaluate(() => {
      ...
      return {
        ...
        sasTarget: [st.sasTarget.x, st.sasTarget.y, st.sasTarget.z, st.sasTarget.w],
      };
    });
    tel = telFromLive(st0, live);
    tel.brain = hasBrain(st0.parts);
    const q = new Quaternion().fromArray(live.quat);
    const tgt = new Quaternion().fromArray(live.sasTarget);
    const err = tgt.multiply(q.clone().invert());
    tel.attErrDeg = 2 * Math.acos(Math.min(1, Math.abs(err.w))) * 180 / Math.PI;
    tel.sas = live.sas;
    tel.throttle = live.throttle;
    tel.tHold = live.t;
...
const result = {
  ...
  setup: 'Kerbin Eye day-side 80 km, SAS hold, 3/4 cam',
  method: 'headed Chrome channel=chrome canvas.toDataURL + HUD colorkey overlay',
  cam: usedCam,
  path: 'logs/shots/sat-s2-hold.png',
  bytes: last?.bytes ?? 0,
...
try { copyFileSync(OUT_PNG, '/workspace/sat-s2-hold.png'); } catch { /* optional */ }
